    ...
    <section id="education" className="py-20 bg-card/30 section-fade-edge scroll-mt-24">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-4xl font-bold text-premium-gradient mb-4 relative inline-block">
            {t('education.title')}
            <span className="absolute -bottom-2 left-1/4 right-1/4 h-1 bg-premium-gradient rounded-full"></span>
          </h2>
        </div>

        <RevealCard className={`group/edu flex flex-col items-center gap-6 hover:border-accent/50 transition-colors p-8 ${lang === 'ar' ? 'md:flex-row-reverse text-right' : 'md:flex-row md:items-start text-center md:text-left'}`}>
          ...
          <div className={`${lang === 'ar' ? 'arabic-text-fix' : ''}`} dir={lang === 'ar' ? 'rtl' : 'ltr'}>
            ...
            <p className="text-slate-400 dark:text-slate-400 font-medium">{t('education.degree')}</p>
          </div>
        </RevealCard>
      </div>
    </section>
